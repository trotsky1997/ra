...
use ra::{
    atif_codec, store::SessionStore, Session, ToolCall, Tool,
...
impl ScriptedModel {
    fn new(turns: Vec<Vec<ModelChunk>>) -> (Self, Arc<Mutex<Vec<Vec<Message>>>>) {
        ...
        (
            Self { turns: Mutex::new(turns), seen: seen.clone() },
            seen,
        )
    }
}
...
async fn resume_preserves_message_log_through_disk() {
    ...
    let turn2 = vec![
        ...
        ModelChunk::End { stop_reason: StopReason::EndTurn },
    ];
    ...
    session_a.prompt("kick off original".to_string()).await.expect("prompt");
    let original_messages = session_a.snapshot_messages().await;
    assert!(original_messages.len() >= 4, "expected at least User/Assistant/ToolResult/Assistant");
    ...
    assert!(saved_path.exists(), "trajectory file must be on disk: {saved_path:?}");
    ...
    session_b.prompt("follow up".to_string()).await.expect("prompt");
    ...
    let saw_original_user = history_seen_by_model.iter().any(|m| {
        matches!(m, Message::User { content } if content == "kick off original")
    });
    assert!(saw_original_user, "resumed session must replay the original user prompt");

    // The original ToolResult must still be there.
    let saw_original_tool_result = history_seen_by_model.iter().any(|m| {
        matches!(m, Message::ToolResult(r) if r.content.contains("echoed: from-original"))
    });
    assert!(
